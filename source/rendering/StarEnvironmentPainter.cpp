...
void EnvironmentPainter::drawRay(float pixelRatio,
  ...
  Vec3B rayColor;
  if (sky.settings.queryBool("sun.dynamicImage", false)) {
    rayColor = jsonToVec3B(sky.settings.query("sun.rayColors." + sky.skyParameters.systemTypeName.value(), sky.settings.query("sun.rayColor", JsonArray{ RayColor[0], RayColor[1], RayColor[2] })));
  } else {
